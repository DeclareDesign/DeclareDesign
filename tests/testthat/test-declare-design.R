...
test_that("No estimators / inquiries", {
  design <-
    ...
    declare_sampling(handler = sampling_handler, S = complete_rs(N, n = 250)) +
    declare_assignment(handler = assignment_handler, Z = complete_ra(N, m = 25)) +
    declare_reveal()
...
test_that("test send design as RHS", {
  my_rhs <- declare_sampling(handler = sampling_handler, S = complete_rs(N, n = 50)) + declare_assignment(handler = assignment_handler, Z = complete_ra(N, m = 5))
