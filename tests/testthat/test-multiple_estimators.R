...
test_that("Two estimators, Two estimands (matched)",{
  ...
  d <- declare_design(
    sleep,
    declare_estimand(CATE_1_5=mean(extra[group == 2]) - mean(extra[group == 1]), subset=ID %in% 1:5),
    declare_estimand(CATE_6_10=mean(extra[group == 2]) - mean(extra[group == 1]), subset=ID %in% 6:10),
    fabricate(extra = extra + rnorm(N)),
    declare_estimator(extra~group, subset=ID %in% 1:5, model=difference_in_means, estimand="CATE_1_5", label="DIM_1_5", coefficient_name = group2),
    declare_estimator(extra~group, subset=ID %in% 6:10, model=difference_in_means, estimand="CATE_6_10", label="DIM_6_10", coefficient_name = group2 )
  )
  diag <- diagnose_design(d)
...
test_that("Two estimators, Two estimands (crossed)",{
  ...
  d <- declare_design(
    ...
    declare_assignment(),
    reveal_outcomes(outcome_variable_names=extra1, assignment_variable_names=Z),
