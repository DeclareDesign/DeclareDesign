...
test_that("Two estimators, Two estimands (matched)",{
  ...
  des <- declare_design(
    ...
    declare_estimator(extra~group, subset=ID %in% 1:5, model=difference_in_means, estimand="CATE_1_5", label="DIM_1_5", coefficients = group2),
    declare_estimator(extra~group, subset=ID %in% 6:10, model=difference_in_means, estimand="CATE_6_10", label="DIM_6_10", coefficients = group2 )
  )
