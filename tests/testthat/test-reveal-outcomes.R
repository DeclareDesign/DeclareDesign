...
test_that("Reveal Outcomes", {
  ...
  my_sampling <- declare_sampling(handler = sampling_handler, S = complete_rs(N, n = 100))

  my_assignment <- declare_assignment(handler = assignment_handler, Z = complete_ra(N, m = 50))
...
test_that("Reveal Outcomes NSE for assignment / outcome variables ", {
  ...
  my_assignment <- declare_assignment(handler = assignment_handler, Z = complete_ra(N, prob = 1))
...
test_that("declare_reveal custom handler works", {
  ...
  my_assignment <- declare_assignment(handler = assignment_handler, Z = complete_ra(N, m = 10))
...
test_that("Single outcome, multiple assn", {
  ...
  assign_A <- declare_assignment(handler = assignment_handler, 
    A = block_ra(blocks = blocks)
  )
  assign_B <- declare_assignment(handler = assignment_handler, 
