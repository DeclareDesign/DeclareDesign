...
test_that("check blocked d-i-m estimator", {
  ...
  my_assignment <- declare_assignment(handler = assignment_handler, Z = block_ra(blocks = blocks))
...
test_that("regression from estimatr works as an estimator", {
  my_population <- declare_population(N = 500, noise = rnorm(N))
  my_potential_outcomes <- declare_potential_outcomes(Y_Z_0 = noise, Y_Z_1 = noise + rnorm(N, mean = 2, sd = 2))
  my_assignment <- declare_assignment(handler = assignment_handler, Z = complete_ra(N, m = 100))
...
sampling <- declare_sampling(handler = sampling_handler, S = complete_rs(N, n = 100))

assignment <- declare_assignment(handler = assignment_handler, Z = complete_ra(N, m = 50))
