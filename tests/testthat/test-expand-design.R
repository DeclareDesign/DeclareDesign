...
test_that("simple designer works", {
  my_designer <- function(N = 100,
    ...
    my_inquiry <- declare_inquiry(mand = my_inquiry_func(Y))
...
test_that("expand with vector arguments", {
  ...
  my_designer <- function(N=10, z = list(1,5,9)) {
    ...
    my_inquiry <- declare_inquiry(mand = max(table(top)))
    my_design <- my_pop + my_inquiry
    my_design
  }
  
  expect_equal(
    draw_inquiries(expand_design(my_designer, z=list(2)))$inquiry, 2)
