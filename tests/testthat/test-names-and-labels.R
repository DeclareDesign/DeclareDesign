...
test_that("estimand labels work",{
  ...
  design <- declare_design(pop, mand, mator)
  names(design)
  ...
  design <- declare_design(pop, mand_2, mator_2)
  names(design)
  diagnosis <- diagnose_design(design, sims = 10, bootstrap_sims = FALSE)
  ...
  design <- declare_design(pop, mand_3, mator_3)
  names(design)
  diagnosis <- diagnose_design(design, sims = 10, bootstrap_sims = FALSE)
  diagnosis$simulations_df
  ...
  design <- declare_design(pop, mand_4, mator_4)
  names(design)
  diagnosis <- diagnose_design(design, sims = 10, bootstrap_sims = FALSE)
  diagnosis$simulations_df
  expect_true(all(diagnosis$simulations_df$estimand_label == "some_stat"))
...
test_that("multiple estimands",{
  ...
  design <- declare_design(pop, mand)
...
test_that("label conflicts",{
  ...
  expect_error(design <- declare_design(pop, mand_1, mand_2))
...
test_that("step name conflicts in design", {
  ...
  mand_1 <- declare_estimand(some_stat = mean(Y))
  expect_error(design <- declare_design(pop, mand_1, mand_1))
  expect_error(design <- declare_design(pop, assign_1, assign_1))
  
  expect_error(design <- declare_design(bob = pop, bob = mand_1))
})
