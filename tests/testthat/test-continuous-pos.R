...
test_that("you can do continuous POs", {
  ...
  my_assignment <- declare_assignment(handler = assignment_handler, Z = complete_ra(N = N, conditions = conditions))
...
test_that("Hooke's law", {
  ...
  # 30 is magic
  sampling <- declare_sampling(handler = sampling_handler, S = complete_rs(N, n = 30))
