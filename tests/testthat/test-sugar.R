...
test_that("+ constructors", {
  d <- declare_population(sleep) + declare_sampling(handler = sampling_handler, S = complete_rs(N)) + declare_assignment(handler = assignment_handler, Z = complete_ra(N, prob = 0.5))
...
test_that("Lots of levels", {
  ...
  design <- declare_population(N = 26000, preference = sample(LETTERS, N, replace = TRUE)) +
    ...
    declare_assignment(handler = assignment_handler, Z = complete_ra(N, conditions = !!LETTERS)) +
    declare_reveal()
