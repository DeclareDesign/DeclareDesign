...
#' design_stub <- population +  potentials + assignment + declare_reveal()
#'
...
#' # ----------
#'
#' # For convenience you can also declare multiple estimands simultaneously and connect these to the corresponding terms for estimates used in the mode. 
#'
...
#' )
#'
#' design_regression <- design_stub + estimands_regression + estimators_regression
#'
...
#' # ----------
#'
#' # You can declare more complex estimands using by writing cutsim estimand functions:
#' 
...
#' }
#' 
#' estimand_custom <- declare_estimand(handler = estimand_function, label = "medianTE")
#'
#' estimand_custom(dat)
#'
...
#'
#' design_cust <- design_stub + estimand_custom + estimator_custom
#' 
...
#'                             c(mean(Y[Z == 0]), mean(Y[Z == 1]) - mean(Y[Z == 0]))),
#'                             term = 1:2)
#' 
...
#'                 diagnosands = declare_diagnosands(select = c(mean_estimate, mean_estimand)))
