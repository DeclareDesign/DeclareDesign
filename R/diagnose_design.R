...
diagnose_design <- function(
  ...
  if (!is.null(add_grouping_variables)) {
    warning(
      "The argument add_grouping_variables is deprecated. Please use make_groups instead.",
      call. = FALSE
    )
  }
  ...
  if (
    !(requireNamespace("future", quietly = TRUE) &
      ...
      (!is.null(future.seed) & future.seed != FALSE)
  ) {
    stop(
      "The future or future.apply packages are not installed. The future.seed argument must be set to FALSE.",
      call. = FALSE
    )
  }
  ...
  if (is.data.frame(..1)) {
    ...
    if (is_empty(c("estimator", "inquiry") %icn% simulations_df)) {
      stop(
        "Can't calculate diagnosands on this data.frame, which does not include either an estimator or an inquiry. Did you send a simulations data frame?"
      )
    }
    if (nrow(simulations_df) == 0) {
      stop(
        "Can't calculate diagnosands on this data.frame, which has zero rows."
      )
    }
  ...
  if (!is.null(make_groups)) {
    ...
    simulations_df <-
      eval_tidy(quo(fabricate(
        simulations_df,
        !!!make_groups[make_groups_has_names]
      )))
  } else {
  ...
  diagnosands_df <- diagnosands_df[
    do.call(order, as.list(diagnosands_df[sort_by_list])),
    ,
    drop = FALSE
  ]
  ...
  if (bootstrap_sims != 0) {
    out$bootstrap_replicates <- merge_param_df(
      bootout$diagnosand_replicates,
      parameters_df
    )
  }
...
setup_diagnosands <- function(..., diagnosands) {
  ...
  diagnosands %||%
    lapply(designs, function(x) attr(x, "diagnosands") %||% default_diagnosands)
}
...
calculate_diagnosands <- function(simulations_df, diagnosands, group_by_set) {
  if ("design" %in% group_by_set) {
    ...
    simulations_list <- split(
      simulations_df,
      lapply(group_by_list, addNA),
      drop = TRUE
    )
...
calculate_diagnosands_single_design <- function(
  simulations_df,
  diagnosands,
  group_by_set
) {
  ...
  diagnosands_df <- as.data.frame(
    t(mapply(c, labels_df, diagnosands_df)),
...
calculate_sims <- function(simulations_df, group_by_set) {
  ...
  n_sims_df <- as.data.frame(
    t(mapply(c, labels_df, n_sims_df)),
    stringsAsFactors = FALSE
  )
...
bootstrap_diagnosands <- function(
  bootstrap_sims,
  future.seed = TRUE,
  simulations_df,
  diagnosands,
  ...
  bootstrap_level <- if ("step_1_draw" %in% names(simulations_df)) {
    "step_1_draw"
  } else {
    "sim_ID"
  }

  boot_indicies_by_id <- split(
    seq_len(nrow(simulations_df)),
  ...
  diagnosand_replicates <- future_lapply(
    seq_len(bootstrap_sims),
  ...
  diagnosands_df <- split(
    diagnosands_df,
    lapply(diagnosands_df_group, addNA),
    drop = TRUE
  )
  ...
  use_vars <- setdiff(
    names(diagnosand_replicates),
    c(group_by_set, "bootstrap_id")
  )
  diagnosands_se_df <- split(
    diagnosand_replicates[use_vars],
    lapply(group_by_list, addNA),
    drop = TRUE
  )
