...
pofdv <- function(design, i, step){
  ...
  check <- function(var_type, step_type, step_attr, callback, from=1, to=length(design)) {
  ...
  unrevealed_outcomes <- check("outcome_variables", "reveal_outcomes", "outcome_variables",
                               ...
                               function(vars){
                                 warning(
                                   "Outcome variables (", paste(vars, sep=", "),
  ...
  if(length(prev_unassigned %i% prev_unrevealed) == 0 && length(unrevealed_outcomes) > 0){
    ...
    warning("Attempting to insert a `", deparse(substitute(declare_reveal(outcome_variables, assignment_variables), this_step_meta)),
  ...
  unassigned_vars <- check("assignment_variables", "assignment", "assignment_variables",
                           from=i+1,
                           function(vars) {
                             warning(
                               "Assignment variables (", paste(vars, sep=", "),
                               ") were declared in a Potential Outcome step (", attr(step, "label"),
  ...
  unrevealed_vars <- check("assignment_variables", "reveal_outcomes", "outcome_variables",
                           from=i+1,
                           function(vars) {
                             warning(
                               "Assignment variables (", paste(vars, sep=", "),
  ...
  if(length(prev_unassigned %i% unassigned_vars %i% unrevealed_vars) == 0 && length(unrevealed_outcomes) > 0){
    ...
    for(step_j in design[length(design):(i+1)]) {
      ...
      if(attr(step_j, "step_type") == "assignment"){
        ...
        if(any(step_meta$assignment_variables %in% attr(step, "step_meta")$assignment_variables)){
          warning("Attempting to insert a `", deparse(substitute(declare_reveal(outcome_variables, assignment_variables), this_step_meta)),
                  "` step after assignment (", attr(step_j, "label"),
                  ")", call.=FALSE)
        ...
        if(any(step_meta$assignment_variables %in% attr(step, "step_meta")$outcome_variables)){
          warning("Attempting to insert a `", deparse(substitute(declare_reveal(outcome_variables, assignment_variables), this_step_meta)),
                  "` step after reveal_step (", attr(step_j, "label"),
                  ")", call.=FALSE)
          design <- insert_step(design, new_step, after = step_j)
          break;
        }
