...
pofdv <- function(design, i, step){
  ...
  # TODO: add labels when you insert a step; make sure it doesn't add duplicate labels; don't clobber existing labels
  # is_generated <- lapply(design, function(x) attr(x, "auto-generated"))
  # 
  # if (any(is_generated)) {
  #   names(design)[is_generated & names(design) == ""] <- paste0("outcomes_generated_", 1:sum(is_generated))
  # }
