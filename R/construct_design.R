...
`+.dd` <- function(lhs, rhs){
  ...
  if (is_missing(rhs) || is_null(rhs)) {
    ...
    if (inherits(lhs, "design")) {
      ...
      qs_rhs <- quos(rhs)
      names(qs_rhs) <- expr_deparse(enexpr(rhs), width = 60L)[1]
      
      qs <- c(qs_lhs, qs_rhs)
      
    } else {
      qs <- c(quo(lhs), quo(rhs))
      names(qs) <- c(expr_deparse(enexpr(lhs), width = 60L)[1], expr_deparse(enexpr(rhs), width = 60L)[1])
    }
  }
