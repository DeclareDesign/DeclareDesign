...
```{r echo=TRUE, results="hide"}
my_potential_outcomes_alt <-
  declare_potential_outcomes(
    Y_Z_0 = .05,
    Y_Z_1 = .30 + .01 * age)

head(my_potential_outcomes_alt(pop))
```
```{r echo=FALSE}
knitr::kable(head(my_potential_outcomes(pop)))
```
...
```{r echo=TRUE, results="hide"}
design <- my_population + my_potential_outcomes + my_estimand + my_sampling + my_assignment + my_estimator_dim
```
