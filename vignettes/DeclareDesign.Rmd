...
1. `declare_population()` (describes the context of the design, as a distribution)
2. `declare_potential_outcomes()` (takes a context and adds potential outcomes) 
...
Multi-level datasets are also easy to produce. For multilevel data you set the `N` of each level in a call to [`level()`](http://fabricatr.declaredesign.org/reference/level.html). The [`level()`](http://fabricatr.declaredesign.org/reference/level.html) function is neat -- if the previous level has data, it merges so that there are N entries for each of the units at the higher level. `declare_population()`can handle non-fixed number of units at each level too.  In the example below, we have  drawn a random number of individuals that are in each village (see the `individuals` line).
...
Next, we recommend you read the [Custom functions](/articles/custom_functions.html) tutorial to learn how to provide custom functions to replace any step of `declare_design()`. You can also read about [`quick_design()` and other advanced features](/articles/advanced_featurs.html) in our Advanced features tutorial.
