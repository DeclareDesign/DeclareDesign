...
**estimatr** provides a small set of commonly-used estimators (methods for estimating quantities of interest like treatment effects or regression parameters), with simple, accessible syntax. We include two functions that implement means estimators, [`difference_in_means()`](http://declaredesign.org/R/estimatr/reference/difference_in_means.html) and [`horvitz_thompson()`](http://declaredesign.org/R/estimatr/reference/horvitz_thompson.html). In addition, we include two functions for linear regression estimators, [`lm_robust()`](http://declaredesign.org/R/estimatr/reference/lm_robust.html) and [`lm_lin()`](http://declaredesign.org/R/estimatr/reference/lm_lin.html). In each case, scientists can choose an estimator to reflect cluster-randomized, block-randomized, and block-and-cluster-randomized designs.
