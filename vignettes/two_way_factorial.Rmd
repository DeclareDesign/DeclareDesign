...
- **I**nquiry: 
    ...
    We wish to know the true average interaction, $E[\beta_{AB}]$. Note $Y_{AB} + Y_0 = \beta_A + \beta_B + \beta_{AB} + 2\epsilon$, and $Y_{A} + Y_{B} = \beta_A + \beta_B + 2\epsilon$. Thus, the expected difference of these terms is equal to the interaction: $E[(Y_{AB} + Y_0) - (Y_A + Y_B)] = E[\beta_{AB}]$.  
...
Note, however, that the power of this design is very low. Even with 1000 subjects, at `r power`% power, whether or not we detect the reasonably large one-quarter standard deviation effect is essentially a coin toss. Half of the time we run the risk of erroneously concluding that the treatments have no effect overall, whereas in combination they do in fact produce a fairly large effect.
